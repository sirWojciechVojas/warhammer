...
class ChatsModel extends Model {
	...
	public function getSorTCurrent($tbl,$tblAll) {
		$tabela = $this->db->table('w_bg_'.$tbl.'_current')->orderBy('NAME','DESC')->join('w_'.$tblAll,'w_bg_'.$tbl.'_current.NAME = w_'.$tblAll.'.ID','left')->getWhere(['BGNAME'=>$this->ID])->getResultArray();
