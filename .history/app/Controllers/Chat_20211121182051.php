...
Class Chat extends BaseController {
	...
	public function index() {
		...
		if ($this->session->get('isLoggedIn') == FALSE) {
			...
			$data['invItem'] = $this->invItem();
			//$this->printr($this->chats->dupa());
			$this->printr($data['invItem']);
/*
...
*/
			...
			$data['a'] = 0;
			$data['mGoldPanel'] = $this->mGold->mGoldPanel('Bretonia','BG',$data['mGold']);
	...
	public function invItem(){
		...
		for($i=0;$i<20;$i++){
				...
				if($i<9) $preV = 'v000';
				if($i>9 && $i<99) $preV = 'v00';
				if($i>99 && $i<999) $preV = 'v0';
				else $preV = 'v';

				$outputTbl[$j][$i] =  $preX.($i+1).$preY.($j+1);
		}
	...
	//	$data['curCareer'] = $this->chats->getProfesje($data['BG']['PREVCAREER_IDs']);
		...
		$this->awansLoop($data['aTalents'],$getTalentsCurrent,'zd',0);
		return redirect()->to(base_url('chat'));
		//$elegy[0][0]=15;
