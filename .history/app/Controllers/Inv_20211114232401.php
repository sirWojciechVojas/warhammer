...
class Inv extends BaseController
{
	...
	public function tradingGM() {
		...
		$data['tempId'] = ($this->request->getPost('idTemp')!==null) ? $invModel->getInvTempId($this->request->getPost('idTemp')) : $invModel->getInvTempId(1);
		$data['indId'] = ($this->request->getPost('idTemp')!==null) ? $invBGModel->getInvTempId($this->request->getPost('idTemp')) : $invBGModel->getInvTempId(1);
		$data['nClass'] = ($this->request->getPost('nClass')!==null) ? $this->request->getPost('nClass') : null;
		// $data['controller'] = $this;
		$outputTbl  = new stdClass();
		...
		$this->view->setVar('tempFieldNames', $invModel->getInvTempNames())
				   ...
				   ->setVar('acc', 'GM')
				   ->setVar('nClass', $data['nClass'])
				   ->setVar('tempId', $data['tempId'])
				   ->setVar('indId', $data['indId'])
				   ->setVar('controller', $this);
		...
		$outputTbl->pip  = $this->request->getPost('idTemp');
