...
class Trading extends BaseController
{
	...
	public function tGoods() {
		...
		$this->view->setVar('tGoods', $this->chats->getInvBG())
					// ->setVar('controller', $this->request->getPost('content'))
					->setVar('controller', $this);
		$outputTbl->tGInd = $this->view->render('tGInd');
		$outputTbl->tFlankL = $this->view->setVar('tGoods', 'left')->render('tFlank');
		$outputTbl->tFlankR = $this->view->setVar('tGoods', 'right')->render('tFlank');
