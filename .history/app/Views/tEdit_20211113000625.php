<?php ob_clean(); foreach($equipFieldNames as $key => $val):
    ...
    <?php elseif ($val == 'DESCRIPTION' || $val == 'DETAILS'):?>
        ...
        <div class="form-row align-items-center col-md-10 pm-0">
            <textarea type="text" class="form-control w-100 noR" id="<?=$val?>" name="fav_language" rows="<?=(8-$key)?>"><?=$invTemp->{$val}?></textarea>
        </div>
    <?php elseif ($val == 'ITEM_CLASS' || $val == 'IMG_CLASS'):?>
        <div class="col-md-2">
            <label for="NAME"><?=$val?></label>
        </div>
        <div class="form-row align-items-center col-md-4 pm-0">
            <input type="text" class="form-control-sm col-md-10" id="<?=$val?>" name="fav_language" value="<?=$invTemp->{$val}?>">
        ...
        <div class="form-row align-items-center col-md-4 pm-0<?=$in?>">
            <input type="text" class="form-control-sm col-md-10" id="<?=$val?>" name="fav_language" value="<?=$invTemp->{$val}?>">
            <input type="button" class="btn btn-outline-info btn-sm col-md-2" id="btn<?=$val?>" name="fav_language" value=">" >
        </div>
    <?php else: ?>
    ...
    <div class="form-row align-items-center col-md-4 pm-0">
        <input type="number" class="form-control-sm w-100" id="<?=$val?>" name="fav_language" value="<?=$invTemp->{$val}?>">
    </div>
