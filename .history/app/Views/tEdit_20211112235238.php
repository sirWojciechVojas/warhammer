<?php ob_clean(); foreach($equipFieldNames as $key => $val):
    if ($val == 'ID' || $val == 'NAME'):?>
        <div class="form-floating col-md-2 outline">
            <label for="<?=$val?>"><?=$val?></label>
        </div>
        <div class="form-row outline col-md-4 pm-0">
    ...
    <?php elseif ($val == 'DESCRIPTION' || $val == 'DETAILS'):?>
        ...
        <div class="form-row align-items-center outline col-md-10 pm-0">
            <textarea type="text" class="form-control w-100 noR" id="<?=$val?>" name="fav_language" rows="5"><?=$invTemp->{$val}?></textarea>
        </div>
