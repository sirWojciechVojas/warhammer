...
<?php foreach($tempFieldNames as $key => $val):
    ...
    <div class="form-row col-md-<?=(2+5*$key)?> pm-0">
        <input type="text" class="form-control-sm w-100" id="<?=$val?>" name="<?=$val?>" value="<?=$idTemp->{$val}?>">
    </div>
    <?php elseif ($val == 'DESCRIPTION' || $val == 'DETAILS'):?>
    <div class="form-flating col-md-12">
        <label for="NAME"><?=$val?></label>
        <textarea type="text" class="form-control w-100 noR" id="<?=$val?>" name="<?=$val?>" rows="<?=(8-$key)?>"><?=$idTemp->{$val}?></textarea>
    </div>
    ...
    <div class="form-row align-items-center col-md-3 pm-0">
        <input type="text" class="form-control-sm col-md-10" id="<?=$val?>" name="<?=$val?>" value="<?=$idTemp->{$val}?>">
        <input type="button" class="btn btn-outline-info btn-sm col-md-2" id="btn<?=$val?>" name="fav_language" value=">">
    </div>
    <?php elseif ($val == 'ITEM_ID'):?>
    <?php $in = ($idTemp->ITEM_CLASS!=="WEAPON") ? ' invisible': null; ?>
