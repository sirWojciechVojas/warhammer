    	...
    	<div class="modal-content" id="HPMenu">
      		...
      		<div class="modal-body<?=$addClass?>" style="padding:0 15px;">
				...
				<?php elseif($prefix == 'Inv'): ?>
					<div class="col-md-2 predesc m-1"><img src="<?= base_url('../warhammer/assets/img/inventory/Inventory_L[72x72]/' . $item['IMG_CLASS'] . '.png') ?>"/></div>
					<div class="col-md-10 predesc m-1">
						<?php if($item['ITEM_CLASS'] == 'WEAPON'): ?>
						<div><?=$item['HANDED']?></div>
						<?php else: ?>
						<div><b>Obciążenie: </b><?=$item['CHARGE']?> jednostek</div>
						<?php endif ?>
						<div><b>Wartość: </b><?=$item['PRIZE']?> brass</div>
					</div>
					<?php if($item['ITEM_CLASS'] == 'WEAPON'): ?>
