<?php ob_clean(); foreach($equipFieldNames as $key => $val):
    ...
    <?php elseif ($val == 'ITEM_CLASS' || $val == 'IMG_CLASS'):?>
        ...
        <div class="form-row align-items-center col-md-4 pm-0">
            <input type="text" class="form-control-sm col-md-11" id="<?=$val?>" name="fav_language" value="<?=$invTemp->{$val}?>">
            <input type="button" class="btn btn-outline-info btn-sm col-md-1" id="btn<?=$val?>" name="fav_language" value=">">
        </div>
