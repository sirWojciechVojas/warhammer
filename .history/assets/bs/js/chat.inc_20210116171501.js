		...
		$.ajax({
			...
			success: function(response){
			  ...
			  // Add response in Modal body
				...
				$('#Brass-line .crown + div input').val(0+' zk');//Bretonia
...
function executeBrass(){
	...
	$('.btn-group').on('click','.btn-warning',function(){
		...
		if($(this).hasClass('addiction')) {
			...
			var e = {'crown':[Math.floor(Brass/240),Math.ceil(Brass/240),' zf'],'shilling':[Math.floor((Brass%240)/12),Math.ceil((Brass%240)/12),' sg'],'brass':[(Brass%240)%12,(Brass%240)%12,' mp']};
