jQuery(function()
{
	...
	$('#trading').on('click','input[value=">"]',function (e) {
		$what = $(this).parent().prev().find('label').text();
		...
		$.ajax({
			...
			// dataType: 'json',
			beforeSend: function() {
				if($what=='IMG_CLASS') $('#trading .modal-footer .row').html($spinner);
				else $('#tradingSell').parent().next().html($spinner);
				// $('#loader').show();
			},
			success: function(response){
				var response = JSON.parse(response);
				if($what=='IMG_CLASS') $('#trading .modal-footer .row').html(response.tClassEdit);
				else $('#tradingSell').parent().next().html(response.tClassEdit);
			}
		});
		return false;
	});
	$('#trading').on('click','.tradingGood',function (e) {
		...
		if($this.hasClass('template') || $this.hasClass('trashInd')){
			...
			if($(this).hasClass('act')) $('#tBuyBtn').removeClass('disabled');
			else $('#tBuyBtn').addClass('disabled');
		}
	...
	$('#trading').on('click','.imgClass',function (e) {

		var imgClass = $(this).attr('title');
		$('#tradingSell input[name="IMG_CLASS"]').val(imgClass);
	});
	$('#trading').on('click','#tBuyBtn, #tSellBtn',function (e) {
		// alert($(this).text());
		if($(this).text()=='Spersonalizuj' || $(this).text()=='Sklonuj'){
			var idTemp = $(this).parent().find('.tradingGood.act input.idTemp').val();
			$this=$(this);
			if($(this).text()=='Spersonalizuj') {var inv = 'invBG'; $tedit='tTrash';}
			else if($(this).text()=='Sklonuj'){ var inv = 'inv'; $tedit='tAdd';}
			$.ajax({
				url: inv+'/new',
				type: 'POST',
				...
				// dataType: 'json',
				beforeSend: function() {
					if($(this).text()=='Sklonuj') $('#tradingBuy').html($spinner);
					$('#tradingSell').html($spinner);
					$thisTxt=$this.text();
		...
		else if($(this).text()=='Usuń'){
			var idInd = $(this).parent().find('.tradingGood.act input.idInd').val();
			$this=$(this);

			$.ajax({
				url: 'invBG/delete',
				type: 'POST',
				data: {prefix: 'Inv', ID: idInd},
				// dataType: 'json',
				beforeSend: function() {
					$('#tradingBuy').html($spinner);
					$('#tradingSell').html($spinner);
