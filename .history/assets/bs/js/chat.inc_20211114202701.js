jQuery(function()
{
	...
	$('#trading').on('click','#tBuyBtn',function (e) {
		var idTemp = $(this).parent().find('input.idTemp').val();
		$.ajax({
			url: 'invBG/new',
			type: 'POST',
			...
			beforeSend: function() {
				$('#tradingBuy').html($spinner);
				$('#tradingSell').html($spinner);
				$('#tBuyBtn').addClass('disabled').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>\
