jQuery(function()
{
	...
	$('#trading').on('shown.bs.modal', function (e) {
		...
		$.ajax({
			...
			/*dataType:"json",*/
			beforeSend: function() {
				$('#tradingBuy').parent().prev().html('<div class="spinner-border text-info"></div>');
	...
	$('#trading').on('click','.GM,.leftGM', function (e) {
		...
		$this = $(this);
		if($this.index()==1) {$tsell = 'tTrash'; $tsellTitle='Stos';}
		// else if($this.index()==2) $tsell = tGInd;
		else if($this.index()==2) {$tsell = 'tTrash'; $tsellTitle='Stos';}
		else {$tsell = 'tAdd'; $tsellTitle='Dodaj/edytuj';}
		...
		$.ajax({
			...
			beforeSend: function() {
				$('#tradingBuy').html('<div class="spinner-border text-info"></div>');
				$('#tBuyBtn').text('Sklonuj');
				$('#tradingSell').html('<div class="spinner-border text-info"></div>');
				$('#tSellBtn').text('Zapisz');
				// $('#loader').show();
			},
			success: function(data){
				var data = JSON.parse(data);
	...
	$('#trading').on('click','#tSellBtn',function (e) {
		var co = $(this).parent().find('form[name="data"]').serializeArray().reduce((acc, {name, value}) => ({...acc, [name]: value}),{});
		$.ajax({
			url: 'inv/update',
			type: 'POST',
			data: {prefix: 'Inv', co: co},
			// dataType: 'json',
			beforeSend: function() {
				$('#tradingBuy').html('<div class="spinner-border text-info"></div>');
				$('#tradingSell').html('<div class="spinner-border text-info"></div>');
				$('#tSellBtn').addClass('disabled').html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>\
			...
			success: function(response){
				...
				$('#tSellBtn').removeClass('disabled').text('Zapisz');
			}
		});
		return false;
	});
