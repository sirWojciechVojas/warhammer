...
<?php if($acc == 'GM' && $side == 'left'):?>
        <button class="btn btn-secondary square justify-content-center <?=$side.$acc?> active">Szablony-dodaj/edytuj</button>
        <button class="btn btn-info square justify-content-center <?=$side.$acc?>">Stos - spersonalizuj</button>
        <button class="btn btn-warning square justify-content-center <?=$side.$acc?>">Ekwipunek BG - spersonalizuj</button>
