		...
		<div class="col-md-6 offset-md-3 d-flex justify-content-center align-self-center">
				...
				<div class="card card-height" style="margin-top:4%;">
					...
					<div class="card-header d-flex justify-content-between">
						...
						<a href="<?= base_url('login/logout') ?>" class="btn btn-danger btn-sm pull-right order-3 d-flex align-items-center">WYLOGUJ</a>
						<?php if ($session->get('akses') == 'ADMIN'): ?>
								<a href="<?= base_url('chat/pending')?>" class="btn btn-primary btn-sm order-1 d-flex align-items-center glyph-icon" title="Zarządzanie użytkownikami">
								...
								<?php if($s->status == TRUE): ?>
									<a href="<?=base_url('chat/maintenance')?>" class="btn btn-warning btn-sm order-2 d-flex align-items-center glyph-icon" title="Wyłącz Chat">
								...
								<?php else: ?>
									<a href="<?=base_url('chat/open')?>" class="btn btn-success btn-sm order-2 d-flex align-items-center glyph-icon" title="Włącz Chat">
										<svg xmlns="http://www.w3.org/2000/svg" class="si-glyph-lock-unlock">
											<use xlink:href="<?=base_url('../warhammer/assets/bs')?>/sprite.svg#si-glyph-lock-unlock" />
										</svg>
									</a>
								<?php endif; ?>
						<?php endif; ?>
					...
					<?php if($ajax==true) ob_clean(); foreach ($chat as $c){ ?>
						...
						<div class="row">
							<div class="col-md-1 justify-content-center align-self-center">
								<img data-toggle="tooltip" data-placement="left" title="<?=$session->get('role')?><br>(<?=$session->get('user')?>)" class="profile-img-mini" src="<?= base_url('../warhammer/assets/img/'.$session->get('role').'.png') ?>" alt="Avatar">
							</div>
							<div class="col-md-11">
						...
						<div class="row">
							<div class="col-md-1 justify-content-center align-self-center">
								<img data-toggle="tooltip" data-placement="left" title="<?=$c->role?><br>(<?=$c->user?>)" class="profile-img-mini" src="<?= base_url('../warhammer/assets/img/'.$c->role.'.png') ?>" alt="Avatar">
							</div>
							<div class="col-md-11">
								<div class="card card-comment">
									<div class="card-header" >
										<strong style="font-size: 12px; color: #076099"><?= $c->user ?>:</strong>
										<small><?php echo date("d.m.Y H:i:s", strtotime($c->waktu)); ?></small><br/>
										<?= $c->teks ?>
									</div>
								</div>
				...
				<?php if ($s->status == TRUE): ?>
					...
					<div class="btn-group d-flex justify-content-between">
						...
						<?php endforeach ?>
						<input id="bountify" type="hidden" value="<?= base_url('../warhammer/assets/bs/js')?>/bountify.inc.js">
					</div>
				<?php endif ?>
			</div>
		</div>
      		...
      		<div class="modal-body cP" style="padding:0 15px;">
				<div class="row h-25 d-flex">
					<div class="col-md-5" style="border:1px solid #fff">
						<div class="col-md-12 opis">Poprzednie profesje:</span></div><div class="col-md-6"><span><?=$prevCareer['NAME']?></span></div>
						<div class="col-md-12 opis">Aktualna profesja:</div><div class="col-md-6"><span><?=$curCareer['NAME']?></span></div>
						<div class="col-md-12 awansListLtr list-group" id="list-tab" role="tablist">
							...
							?>
							<a class="awansList ist-group-item list-group-item-action" id="list-<?=$oProf?>-list" data-toggle="list" href="#list-<?=$oProf?>" role="tab" aria-controls="<?=$oProf?>"><?=$oProf?></a>
							<?php endforeach ?>
					</div>
					</div>
					<div class="col-md-7 tab-content" id="nav-tabContent" style="border:1px solid #fff">
						...
						//$sec = (preg_match('#różne#',$talent['NAME'])) ? ' secList' : null;
						?>
						<div class="tab-pane fade" id="list-<?=$oProf?>" role="tabpanel" aria-labelledby="list-<?=$oProf?>-list">Twoja nowa profesja: <?=$oProf?></div>
						<?php endforeach ?>
					</div>
	...
	<div class="row h-100 d-flex">
		<div class="col-md-4 cPanel cLeft">
			<div class="row h-100">
				<div class="col-md-3 iSide d-flex">
					<div class="inventory-table mx-auto">
						<div class="inventory-row severalSlots"  data-item-filter-whitelist="head">
							<div class="inventory-cell Head"></div>
						</div>
						<div class="inventory-row severalSlots">
				...
				<div class="col-md-3 iSide d-flex">
					<div class="inventory-table mx-auto">
						<div class="inventory-row severalSlots" data-item-filter-whitelist="cape">
							<div class="inventory-cell Cape"></div>
						</div>
						<div class="inventory-row severalSlots" data-item-filter-whitelist="armor">
							<div class="inventory-cell Body"></div>
						</div>
						<div class="inventory-row severalSlots">
							<div class="inventory-cell"></div>
						</div>
				...
				<div class="col-md-10 offset-md-1 imBottom">
					...
					<div class="col-md-12 d-flex justify-content-start" style="padding:0;">
						<div class="crown"></div><div><input type="text" readonly="true" value="<?=$mGold->mCrown?> zk"></div>
						<div class="shilling"></div><div><input type="text" readonly="true" value="<?=$mGold->mShilling?> s"></div>
						<div class="brass"></div><div><input type="text" readonly="true" value="<?=$mGold->mPenny?> p"></div>
						<input type="hidden" id="hBrass" value="<?=$mGold->hBrass?>"/>
					</div>
					<div class="col-md-12 d-flex justify-content-around" style="padding:0;">
						<div><input type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-symbol="+" value="Pozyskaj"></div>
						<div><input type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-symbol="-" value="Wydaj"></div>
					</div>
				</div>
				<div class="iBottom mx-auto d-flex justify-content-center align-items-center" style="width:436px;">
		...
		<div class="col-md-4 cPanel cCenter">
			<div class="row h-100 d-flex align-content-start flex-wrap">
				...
				<div class="col-md-5 offset-md-1 cechy cechyGlowne d-flex justify-content-between flex-column" data-button="Wykup">
				...
				<?php if($Glowne[2][$k]<$Glowne[0][$k1]+$Glowne[1][$k2]): $klasa=null; else: $klasa=' disabled'; endif?>
					<div class="d-flex justify-content-between">
						...
						<input type="button" data-key="<?=$k?>" data-toggle="modal" class="btn btn-danger<?=$klasa?>" value="+5"/>
					</div>
				<?php endforeach ?>
				</div>
				...
				<?php if($Drugo1[2][$k]<$Drugo1[0][$k1]+$Drugo1[1][$k2]): $klasa=null; else: $klasa=' disabled'; endif?>
					<div class="d-flex justify-content-between">
						...
						<input type="button" data-key="<?=$k?>" data-toggle="modal" class="btn btn-danger<?=$klasa?>" value="+1"/>
					</div>
				<?php endforeach ?>
				</div>
				...
				<?php foreach($allMySkills as $k => $val):?>
					<?php if((int) $val['STATUS']!==0): ?>
						<div class="skill-talent" data-toggle="tooltip" data-placement="left" title="<?=htmlspecialchars('<h3><b>'.$val['NAME'].'</b></h3><h4>'.$val['DESCRIPTION'].'</h4>')?>"><input type="text" readonly="true" class="L-<?=$val['STATUS']?>"/><?=$val['NAME']?></div>
					<?php endif ?>
				<?php endforeach ?>
				</div>
				...
				<?php foreach($allMyTalents as $k => $val):?>
					<?php if((int) $val['STATUS']!==0): ?>
						<div class="skill-talent" data-toggle="tooltip" data-placement="right" title="<?=htmlspecialchars('<h3><b>'.$val['NAME'].'</b></h3><h4>'.$val['DESCRIPTION'].'</h4>')?>"><input type="text" readonly="true" class="R-<?=$val['STATUS']?>"/><?=$val['NAME']?></div>
					<?php endif ?>
				<?php endforeach ?>
				</div>
			</div>
		...
		<div class="col-md-4 cPanel cRight">
			...
			<div class="row">
				<div class="col-md-12 diaryTitle">Dodatkowe Informacje</div>
				<div class="col-md-12 history diary">
					<ul>
					<?php foreach($diaryLi as $val):?>
						<!-- <li><?=$val->updated_at.': '.$val->MESSAGE?></li> -->
						<li><?=$val->MESSAGE?></li>
					<?php endforeach ?>
					</ul>
				</div>
				<div class="d-flex">
					<input type="text" name="msgDiary" class="form-control" placeholder="Tu wprowadź tekst">
					<div class="input-group-btn"><input class="btn btn-success btn-sm" type="submit" value="Zanotuj"></div>
				</div>
			</div>
		</div>
	</div>
...
<div id="skillsPanel">
	...
	<div class="row abbRow" data-button="Wykup">
		...
		?>
		<div class="abbList<?=$sec?>" data-toggle="tooltip" data-placement="right" data-details="<?=$skill['DETAILS']?>" data-id="<?=$skill['ID']?>" title="<?=htmlspecialchars('<h3><b>'.$skill['NAME'].'</b></h3><h4>'.$skill['DESCRIPTION'].'</h4>')?>"><?=$skill['NAME']?></div>
		<?php endforeach ?>
	    </div>
	</div>
	<div class="row">
...
<div id="characterPanel">
	...
	<div class="row background d-flex align-items-start justify-content-between">
		...
		<div class="row col-md-6 titleBar">
			<div class="col-md-12 HPBarPlace"><div class="col-md-12 HPBar" style="width:<?=$HP->HPpercent?>%;"></div></div>
			<div style="width:6.75%;" data-toggle="modal" data-target="#HPModalCenter" data-symbol="0">-</div>
			<div style="width:86%;" class="tPlace" data-toggle="tooltip" data-placement="top" data-original-title="<?=htmlspecialchars('<h5><b>Punkty Żywotności</b></h5><h6>'.$HP->HP.'/'.$HP->WOUNDS.'</h6>')?>"><?=$session->get('USEDNAME')?></div>
			<div style="width:6.75%;" data-toggle="modal" data-target="#HPModalCenter" data-symbol="1">+</div>
		</div>
		<div class="col-md-3 titleSpace" align="right"><button class="btn btn-danger btn-sm awans disabled" data-toggle="modal" data-target="#ModalCenter">Awansuj</button></div>
		...
		<div class="col-md-1 titleSpace" align="left"><button class="btn btn-success btn-sm disabled" data-toggle="modal" data-symbol="0" data-target="#PDModalCenter">-</button></div>
		<div class="col-md-10" style="padding:0;margin:0;">
			<div class="col-md-12 PDBar d-flex" data-toggle="tooltip" data-placement="top" title="<?=htmlspecialchars('<h6><b>Punkty Doświadczenia</b></h6><h6><u>Pasek kompleksowy</u></h6><h6>'.$PD[1].'/'.$PD[3].' PD<br>(niewykorzytane: '.$PD[5].' PD)</h6>')?>">
			...
			<div class="col-md-12 PDBar d-flex" data-toggle="tooltip" data-placement="top" title="<?=htmlspecialchars('<h6><b>Punkty Doświadczenia</b></h6><h6><u>Pasek minimalny</u></h6><h6>'.$PD[4].'/'.$PD[2].' PD<br>(niewykorzytane: '.$PD[5].' PD)</h6>')?>">
				<div class="PD-4" style="width:<?=$PD[8]?>%;"></div>
				<div class="PD-5" style="width:<?=$PD[9]?>%;"></div>
			</div>
		</div>
		<div class="col-md-1 titleSpace" align="right"><button class="btn btn-success btn-sm" data-toggle="modal" data-symbol="1" data-target="#PDModalCenter">+</button></div>
	</div>
</div>
<?php if($session->get('akses')=='ADMIN'): ?>
<div id="characterChanger">
	<div class="row">
		<div class="col-md-12">
		...
		<div class="col-md-12">
			...
			<?php foreach($BGs as $id):
				...
				<li class="<?=$activ?>" data-id="<?=$id->ID;?>" data-usedname="<?=$id->USEDNAME;?>">
					<img data-toggle="tooltip" data-placement="bottom" title="ID: <span><?=$id->ID;?></span><br><?=$id->USEDNAME;?><br>(<?=$id->GAMER_NAME;?>)" class="profile-img-mid" src="<?= base_url("../warhammer/assets/img/$id->USEDNAME.png") ?>" alt="Avatar">
				</li>
			<?php endforeach ?>
			</ul>
		</div>
